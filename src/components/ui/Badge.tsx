...
function Badge({ className, variant = "default", ...props }: BadgeProps) {
    ...
    return (
        <div
            className={cn(
                "inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider transition-all focus:outline-none focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 shadow-sm",
                variants[variant],
                className
            )}
            {...props}
